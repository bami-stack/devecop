...
echo "Report:   $REPORT_PATH"#!/usr/bin/env bash
# Security Scan Runner - one repo per execution (for n8n loop integration)
set -euo pipefail
...
# --- Resolve input from env or input.json ---
if [[ -z "$REPO_PATH" && -n "${SECURITY_SCAN_REPO_PATH:-}" ]]; then
  REPO_PATH="$SECURITY_SCAN_REPO_PATH"
fi
...
if [[ -z "$REPO_PATH" ]]; then
  INPUT_FILE="$WORKFLOW_DIR/input.json"
  if [[ -f "$INPUT_FILE" ]]; then
    APPROVED=$(jq -r '.approved // false' "$INPUT_FILE")
    if [[ "$APPROVED" != "true" ]]; then
      echo "ERROR: approved is false or missing. Aborting." >&2
      exit 1
    fi
    REPO_PATH=$(jq -r '.repoPath // empty' "$INPUT_FILE")
    REPO_URL=$(jq -r '.repoUrl // empty'   "$INPUT_FILE")
    [[ -z "$REPO_NAME" ]] && REPO_NAME=$(jq -r '.repoName // empty' "$INPUT_FILE")
    SCAN_CONTAINERS=$(jq -r '.scanContainers // true' "$INPUT_FILE")

    # Clone/pull if repoPath missing but repoUrl provided
    if [[ -z "$REPO_PATH" && -n "$REPO_URL" ]]; then
      CLONE_DIR="$WORKFLOW_DIR/_repos/${REPO_NAME:-$(basename "$REPO_URL" .git)}"
      if [[ -d "$CLONE_DIR/.git" ]]; then
        echo "Pulling latest: $CLONE_DIR"
...
if [[ -z "$REPO_PATH" ]]; then
  echo "ERROR: Repo path required. Set SECURITY_SCAN_REPO_PATH, pass --repo-path, or set workflow/input.json." >&2
  exit 1
fi
...
REPORT_NAME="security_scan_${SAFE_REPO_NAME}_${DATE}.md"
REPORT_PATH="$REPORTS_DIR/$REPORT_NAME"
GRYPE_CONFIG="$SCRIPTS_DIR/grype-config.yaml"
...
[[ -f "$REPO_PATH/package.json"     ]] && MANIFESTS="$MANIFESTS node"
[[ -f "$REPO_PATH/go.mod"           ]] && MANIFESTS="$MANIFESTS go"
[[ -f "$REPO_PATH/requirements.txt" ]] && MANIFESTS="$MANIFESTS python"
[[ -f "$REPO_PATH/Pipfile"          ]] && MANIFESTS="$MANIFESTS python"
[[ -f "$REPO_PATH/pyproject.toml"   ]] && MANIFESTS="$MANIFESTS python"
[[ -f "$REPO_PATH/pom.xml"          ]] && MANIFESTS="$MANIFESTS maven"
[[ -f "$REPO_PATH/Cargo.toml"       ]] && MANIFESTS="$MANIFESTS rust"
MANIFESTS="${MANIFESTS# }"  # trim leading space

while IFS= read -r -d '' file; do
  rel="${file#$REPO_PATH/}"
  if echo "$rel" | grep -qE 'Dockerfile|docker-compose|k8s|helm|charts|deploy|Chart\.yaml|values\.yaml'; then
    CONTAINER_FILES="$CONTAINER_FILES $rel"
  fi
done < <(find "$REPO_PATH" -type f \
  \( -name "Dockerfile" -o -name "docker-compose*.yml" -o -name "*.yaml" -o -name "*.yml" \) \
  -print0 2>/dev/null)
...
# --- npm audit ---
if echo "$MANIFESTS" | grep -qw "node"; then
  {
    echo "### npm audit"
    echo '```'
  } >> "$REPORT_PATH"
  ...
  if echo "$AUDIT_OUT" | jq -e '.vulnerabilities' > /dev/null 2>&1; then
    while IFS= read -r pkg; do
      sev=$(echo "$AUDIT_OUT" | jq -r --arg p "$pkg" '.vulnerabilities[$p].severity // "unknown"')
      vid=$(echo "$AUDIT_OUT" | jq -r --arg p "$pkg" \
        '.vulnerabilities[$p].via[0].source // .vulnerabilities[$p].via[0] // "unknown"')
      fix_available=$(echo "$AUDIT_OUT" | jq -r --arg p "$pkg" \
        '.vulnerabilities[$p].fixAvailable // false')
      fix="Review manually"
      [[ "$fix_available" == "true" ]] && fix="npm update $pkg"
      append_finding "$vid" "$sev" "package.json" "$pkg" "$fix"
    done < <(echo "$AUDIT_OUT" | jq -r '.vulnerabilities | keys[]')
...
if command -v grype > /dev/null 2>&1 && [[ -f "$GRYPE_CONFIG" ]]; then
  ...
  GRYPE_OUT=$(grype --config "$GRYPE_CONFIG" "$REPO_PATH" -o json 2>&1 || true)
  COMMANDS_RUN="$COMMANDS_RUN\n- grype --config scripts/grype-config.yaml <repo> -o json"
  ...
  if echo "$GRYPE_OUT" | jq -e '.matches' > /dev/null 2>&1; then
    while IFS=$'\t' read -r vid sev name fix_versions; do
      append_finding "$vid" "$sev" "filesystem / SBOM" "$name" "${fix_versions:-no fix available}"
    done < <(echo "$GRYPE_OUT" | jq -r \
      '.matches[] | [
        .vulnerability.id,
...
# --- Summary table ---
{
  ...
  echo "$FINDINGS_JSON" | jq -r \
    '.[] | "| \(.finding) | \(.severity) | \(.location) | \(.affected) | \(.fix) |"'
  echo ""

  echo "## 3) Prioritized fixes (top critical/high)"
  echo "$FINDINGS_JSON" | jq -r \
    '.[] | select(.severity | test("critical|high"; "i")) |
     "- **\(.severity)** \(.finding) in \(.affected): \(.fix)"' | head -5
  echo ""
...
# --- Write output.json for n8n ---
TOTAL_FINDINGS=$(echo "$FINDINGS_JSON" | jq 'length')
COMMANDS_JSON=$(printf '%b' "$COMMANDS_RUN" | grep -v '^$' | sed 's/^- //' | jq -R . | jq -s .)
